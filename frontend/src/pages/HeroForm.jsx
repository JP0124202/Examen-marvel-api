...
export default function HeroForm() {
  ...
  useEffect(() => {
    if (editing) {
      api.get(`/heroes/${id}`).then(r => {
        const d = r.data.data || {}
        setForm({
          nombre: d.nombre || '',
          nombre_real: d.nombre_real || '',
          poder_principal: d.poder_principal || '',
          nivel_poder: d.nivel_poder ?? 1,
          imagen_url: d.imagen_url || d.imagen || '',
          estado: d.estado || 'ACTIVO'
        })
      }).catch(() => setError('No se encontró el héroe'))
    }
  }, [id, editing])
    ...
    <div className="card form">
      ...
      <form onSubmit={handleSubmit}>
        <div className="form-grid">
          ...
          {form.imagen_url && (
            <div className="full">
              <div className="card" style={{padding:8, borderRadius:14}}>
                <img src={form.imagen_url} alt="preview" style={{width:220,height:140,objectFit:'cover',borderRadius:10}} onError={(e)=>e.target.src='https://via.placeholder.com/300x200?text=No+Image'} />
              </div>
            </div>
          )}
        </div>
